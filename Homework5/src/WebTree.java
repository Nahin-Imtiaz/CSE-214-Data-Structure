			...
			if(data.contains("</a>")) {	
				...
				for(int i=0;i<counter && i<3;i++) {
					...
					linkName = str[i].substring(str[i].indexOf(">")+1,
					  str[i].indexOf("</a>"));
					if(file.contains("=")) {
						if(linkName.contains("<a class=")) {
							file=linkName.substring(linkName.indexOf("href=")+6, linkName.indexOf("html")+3);
							linkName=linkName.substring(linkName.indexOf("html")+6, linkName.length());
							newNode.setFilename(file);
							newNode.setLinkName(linkName);
						}
						else {
						file=file.substring(0,fileName.indexOf("html")+3);
						newNode.setFilename(file);
						newNode.setLinkName(linkName);
						}
					}

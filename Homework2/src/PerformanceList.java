 ...
 *
 *
 *This class is used to store objects of PerformanceNode into a linked list.
 *It has head,tail and cursor.It also provide an interface to interact with 
 *the linked list.
 */
public class PerformanceList {
	...
	public void addToEnd(PerformanceNode newPerformance) {
		if(this.head == null||cursor==null){
			...
			tail = newPerformance;
			newPerformance.setStartTime(newPerformance.getPrevLink()
			  .getStartTime(),newPerformance.getPrevLink().getDuration());
			newPerformance.setSerialNo(newPerformance.getPrevLink()
			  .getSerialNo()+1);
		}
		position++;
	}
	/**
	 * Inserts the new data into the PerformanceList such that the new node 
	 * directly follows the current node
	 * @param newPerformance
	 * this is an object of PerformanceNode
	 */
	public void addAfterCurrent(PerformanceNode newPerformance) {
		if(cursor==null) {
			...
			while(newPerformance!=null) {
				newPerformance.setStartTime(newPerformance.getPrevLink().
				  getStartTime()+tempStartTime,newPerformance.getPrevLink()
				  .getDuration()+tempDuration);
				newPerformance.setSerialNo(newPerformance.getPrevLink()
				  .getSerialNo()+1);
	...
	/**
	 * Removes the current node, if it exists. The current node should now be
	 * the node after the one that was just removed.
	 * @return boolean
	 */
		...
		if(cursor!=null ) {
			...
			if(cursor.getNextLink()==null && cursor.getPrevLink()==null) {
				...
				while(tempNode1!=null){
					tempNode1.setStartTime(tempNode1.getPrevLink()
					  .getStartTime()-tempStartTime,tempNode1.getPrevLink()
					  .getDuration()-tempDuration);
					tempNode1.setSerialNo(tempNode1.getPrevLink()
					  .getSerialNo()-1);
					tempNode1 = tempNode1.getNextLink();
				}	
			}
			position--;
			System.out.println("performance "+tempNode.getPerformance()+
			  " has been removed.");
			return true;
		}
		return false;
	...
	public boolean moveCursorForward() throws NoCurrentNodeException {
		if(cursor == null) {
			throw new NoCurrentNodeException("There is no performance in the "
			  + "schedule.");
		}
	...
	public boolean moveCursorBackward() throws NoCurrentNodeException{
		if(cursor == null) {
			throw new NoCurrentNodeException("There is no performance in"
			  + " the schedule.");
		}
	...
	/**
	 * formats all the objects of the PerformanceNode class and saves it as
	 *  a string
	 * @return 

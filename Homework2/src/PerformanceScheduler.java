 ...
 *		Section -06
 *
 *This class contains a main method that presents a menu that allows the user 
 *to access data from the API, store it in a linked list, and interact with 
 *it using commands.
 */
import java.util.Scanner;
public class PerformanceScheduler {
	/**
	 * The main method runs a menu driven application which first creates one
	 *  empty PerformanceList and then prompts the user for a command selecting
	 *   the operation. Once an operation is selected, the program prompts for 
	 *   any additional information required to perform the operation, and then
	...
	public static void main(String[] args) {
		...
		while(i!=0) {
			...
			/**
			 * Prompts the user for information regarding a performance. 
			 * Create a new PerformanceNode for this performance, and add
			...
			if(operation.equalsIgnoreCase("A")) {
				...
				System.out.println(" ");
				System.out.println("New performance "+performance+" is added "
				  + "to the end of the list.");
				System.out.println(" ");
			}
			try {
				/**
				 * Moves the reference of the current node backwards by one 
				 * position if there is a previous node in the list. 
				 */
				if(operation.equalsIgnoreCase("B")) {
					if(list.getCursor().getPrevLink()==null) {
						System.out.println(" ");
						System.out.println("The current slide is the first in"
						  + " the list.");
						System.out.println(" ");
					}
			...
			if(operation.equalsIgnoreCase("C")) {
				if(list.getCursor()==null) {
					System.out.println(" ");
					System.out.println("There is no performance in the"
					  + " schedule.");
					System.out.println(" ");
				}
				else {
					System.out.printf("%-8s%-4s%-25s%-25s%-13s%-9s%-10s%n",
					  "Current","No.","Performance Name","Lead Performer"
			    	  + " Name","Participants","Duration","Start"
					  + " time");
					System.out.printf("%-8s%-4s%-25s%-25s%-13s%-9s%-10s%n",
					  "--------","----","-------------------------","-------"
					  + "------------------","-------------","---------","---"
					  + "-------");
					System.out.printf("%7s  %-3d","*",list.getCursor()
					  .getSerialNo());
			...
			if(operation.equalsIgnoreCase("D")) {
				if(list.getCursor()==null) {
					System.out.println(" ");
					System.out.println("There are no performances in the"
					  + " schedule.");
					System.out.println(" ");
				}
				else{
					System.out.printf("%-8s%-4s%-25s%-25s%-13s%-9s%-10s%n",
					  "Current","No.","Performance Name","Lead Performer "
					  + "Name","Participants","Duration","Start time");
					System.out.printf("%-8s%-4s%-25s%-25s%-13s%-9s%-10s%n",
					  "--------","----","-------------------------","------"
					  + "-------------------","-------------","---------",
					  "----------");
			...
			try {
				/**
				 * Moves the reference of the current node forward by one 
				 * position if there is another node in the list. 
				 */
				if(operation.equalsIgnoreCase("F")) {
					if(list.getCursor().getNextLink()==null) {
						System.out.println(" ");
						System.out.println("The current slide is the last "
						  + "in the list.");
						System.out.println(" ");
					}
			...
			/**
			 * Prompts the user for information regarding a performance. Create
			 *  a new PerformanceNode and add it after the current performance.
			 */
			if(operation.equalsIgnoreCase("I")) {
				...
				System.out.println(" ");
				System.out.println("New performance "+performance+" is added "
				  + "after the current performance.");
				System.out.println(" ");
			}
			/**
			 * Prompts the user for a position number and moves the current 
			 * node to that number node, if it exists. 
			 */
			if(operation.equalsIgnoreCase("J")) {
				...
				if(list.getPosition()<position) {
					System.out.println(" ");
					System.out.println("There is no performance at this"
					  + " position.");
					System.out.println(" ");
				}
			...
			/**
			 * If there is a current node, it is removed from the list and 
			 * the current node is moved to the next appropriate current node. 
			 */
